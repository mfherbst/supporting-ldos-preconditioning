...
function construct_localiser_AlGaAsSiO2H(lattice, atoms; σ=30)
    @assert lattice == Diagonal(diag(lattice))
    @assert maximum(lattice) == lattice[3, 3]  # 3rd axis is the elongated one
    @assert length(atoms) == 6                 # Two types of atoms
    @assert atoms[1][1].symbol == :Ga          # Gallium symbol
    @assert atoms[2][1].symbol == :As          # Arsenic symbol
    @assert σ ≥ 15

    # The slabs are usually from 0.0 through to around 0.333
    # Using this we find first and last Ga and As atom
    last_z  = sort([r[3] for iat in (1, 2) for r in atoms[iat][2]], by=z -> abs(z - 0.3))[1]
...
config[@__DIR__] = (
    ...
    firstmixkey="Custom",
    #
    construct_localiser=construct_localiser_AlGaAsSiO2H,
    mixings=Dict(
